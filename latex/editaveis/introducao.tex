...
Na esteira da transformação ágil, em 2009, XXXX propôs o termo DevOps. Esse termo abarcou um conjunto de nuances que que em essência, buscava remover a barreira existente entre os times de desenvolvimento e operações, quando da implantação. A compreensão sobre esse termo passa por diferentes facetas como: cultura, automação; métodos e procedimentos de desenvolvimento e operações contínuos e integrados; colaboração \cite{10.1145} \cite{10.21203v1} \cite{DBLP:journals/jss/LuzPB19} . Um dos principais objetivos é reduzir o ciclo 
...
lançamento de forma automatizada (RAJAPAKSE et al., 2021). 
...
testes de segurança automatizados e feedback contínuo . \textcolor{blue}{(RAJAPAKSE et al., 2021)}\todo[color=yellow]{Inserir o .bibtex e corrigir a citacao}


\todo[inline, color=pink]{Levar esse parágafo abaixo para a seção o cap. de refencial teórico.}
...
Medir a segurança de software representa um grande desafio \textcolor{blue}{(RAJAPAKSE et al., 2021)}\todo[color=yellow]{Inserir o .bibtex e corrigir a citacao}.  Embora já exista muito conhecimento acumulado na área, ainda carecemos de modelos que apresentem 
...
são impraticáveis devido à velocidade das entregas \textcolor{blue}{(RAJAPAKSE et al., 2021)}\todo[color=yellow]{Inserir o .bibtex e corrigir a citacao}. A medição da seguranca se torna ainda mais desafiadora ao lidar ciclos continuos de lançamentos de versões de produtos. A segurança é uma propriedade multifacetada, 
emergente e dependente do contexto, o que complica sua quantificação . \textcolor{blue}{(KUDRIAVTSEVA; GADYATSKAYA, 2024)}\todo[color=yellow]{Inserir o .bibtex e corrigir a citacao}
...
\todo[inline, color=pink]{Refazer a fig. 1  ou utilizar uma imagem em português. Você pode reaproveitar a Fig. 1 no tcc do Senna, fazendo a citação.}
...
O objetivo geral deste trabalho consiste em analisar a adoção de práticas de DevSecOps no ciclo de devida de desenvolvimento do produto de software-livre \textcolor{blue}{(MEPA}\todo[color=yellow]{Inserir nota de rodapé com a url do MEPA}. Para alcançar este propósito, 
