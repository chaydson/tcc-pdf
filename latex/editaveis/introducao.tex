...
experimentação em Engenharia de \textit{Software}, segurança de produtos de \textit{software} e \textit{DevSecOps}. 
...
processo e reduzir subjetividades \cite{Siavvas2021431}. Nesse sentido, o modelo proposto por 
\citeonline{McCall1977} foi o primeiro modelo hierárquico para analisar a qualidade, 
no qual os diferentes fatores eram analisados por critérios e avaliados por métricas. Subsequentemente, 
o modelo de \citeonline{Boehm1978} evoluiu as ideias de McCall, e ambos se tornaram modelos seminais, 
servindo de referência para os modelos subsequentes.

A partir da \citeonline{ISO9126}, estabeleceu-se um padrão internacional para a qualidade de software que 
decompunha essa qualidade em características e subcaracterísticas, em um modelo hierárquico, além de 
definir os termos técnicos da área. A \citeonline{ISO25010} surgiu como uma evolução da \citeonline{ISO9126}, expandindo
seus conceitos e adaptando o modelo para a nova realidade da qualidade de software moderno. Diferentemente
da \citeonline{ISO9126}, na \citeonline{ISO25010} a segurança já é definida como um dos pilares da qualidade,
e não como uma subcaracterística.

Como padrão internacional de segurança da informação, tem-se a \citeonline{ISO27001}. 
...
Contudo, tanto a \citeonline{ISO25010} quanto a \citeonline{ISO27001} não fornecem um método para avaliar a 
...
O \textit{DevOps} é um paradigma que visa remover as barreiras entre os times de desenvolvimento e operações, 
a fim de construir um ambiente colaborativo e integrado \cite{Rajapakse2022}. Seu objetivo é reduzir o ciclo 
de vida do desenvolvimento de \textit{software}, permitindo entregas mais frequentes. As principais práticas de 
\textit{DevOps} são a Integração Contínua (\textit{CI}), que consiste em integrar o código desenvolvido na ramificação 
principal com validação de \textit{build} e testes de forma automática para detectar falhas, e a Entrega/Implantação
Contínua (\textit{CD}), que consiste em deixar o \textit{software} pronto para entrar em produção e realizar o seu 
...
testes de segurança automatizados e \textit{feedback} contínuo \cite{Rajapakse2022}.
...
Medir a segurança de \textit{software} representa um grande desafio \cite{Rajapakse2022}. A literatura atual carece de modelos que apresentem 
...
confiabilidade dos resultados \cite{Siavvas2021431}.

No contexto \textit{DevOps}, esse desafio se torna ainda mais difícil. Métodos tradicionais de análise de segurança 
são impraticáveis devido à velocidade das entregas \cite{Rajapakse2022}. A medicao da seguranca se torna ainda
...
\begin{table}[h]
    ...
    \begin{tabular}{|p{5cm}|p{9cm}|}
        ...
        \hline
        Analisar & A característica de qualidade de produto de \textit{software}: \textbf{segurança} \\
        \hline
        ...
        \hline
        Com respeito a & Desenvolvimento e operação de produtos de \textit{software} seguros (DevSecOps) \\
        \hline
        Do ponto de vista de & Pesquisador \\
        \hline
        No contexto de & Desenvolvimento de aplicações web seguras (\textit{software} livre, organizações públicas e privadas) \\
        \hline
    \end{tabular} \\[0.5em]
...
\begin{center}
Como analisar a característica de segurança no desenvolvimento contínuo de sistemas \textit{web}, considerando as 
...
\begin{itemize}
    ...
    \item Conduzir o estudo de caso, realizando a coleta e a análise das métricas de qualidade de \textit{software}.
...
\begin{itemize}
    ...
    \item Referencial Teórico: estabelece a fundamentação teórica da monografia, abordando os tópicos centrais da pesquisa: \textit{DevSecOps}, qualidade de \textit{software}, segurança de \textit{software} e modelos de avaliação de maturidade.
...
\begin{itemize}
    \item Contextualização sobre Engenharia de \textit{Software} Experimental: Estudo sobre os métodos de pesquisa empírica em Engenharia de \textit{Software}, como revisão sistemática da literatura, \textit{survey}, experimentos e estudo de caso, fundamentais para a condução do trabalho \cite{Wohlin2024}.
    \item Definição do GQM: Aplicação da abordagem \textit{Goal Question Metric} (GQM) para a construção da questão principal de pesquisa, suas subquestões e as métricas que orientarão a revisão da literatura \cite{Basili1994}.
    \item Elaboração do Protocolo de Revisão: Estruturação de um protocolo de revisão sistemática da literatura para pesquisar, selecionar e analisar os artigos \cite{KITCHENHAM20132049}. Este processo inclui a definição da string de busca (baseada no \textit{framework} PICO), a elaboração de sinônimos, a definição dos critérios de inclusão e exclusão e o método para extração de dados.
