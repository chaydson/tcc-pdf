...
Implementation, Information Gathering e Test and Verification \cite{Lange2024}.

\section{Problema}

Medir a segurança de software representa um grande desafio \cite{Rajapakse2022}. A literatura atual carece de modelos que apresentem 
...
\begin{itemize}
    \item Contextualização sobre Engenharia de Software Experimental: Estudo sobre os métodos de pesquisa empírica em Engenharia de Software, como revisão sistemática da literatura, survey, experimentos e estudo de caso, fundamentais para a condução do trabalho \cite{Wohlin2024}.
    \item Definição do GQM: Aplicação da abordagem Goal Question Metric (GQM) para a construção da questão principal de pesquisa, suas subquestões e as métricas que orientarão a revisão da literatura \cite{Basili1994}.
    \item Elaboração do Protocolo de Revisão: Estruturação de um protocolo de revisão sistemática da literatura para pesquisar, selecionar e analisar os artigos \cite{KITCHENHAM20132049}. Este processo inclui a definição da string de busca (baseada no framework PICO), a elaboração de sinônimos, a definição dos critérios de inclusão e exclusão e o método para extração de dados.
