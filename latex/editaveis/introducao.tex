...
testes de segurança automatizados e opinião contínuo \cite{Rajapakse2022}.
...
O objetivo geral deste trabalho consiste em analisar a adoção de práticas de DevSecOps no ciclo de devida de desenvolvimento do produto de software-livre \textcolor{blue}{Brasil Participativo}\footnote{\href{https://gitlab.com/lappis-unb/decidimbr/decidim-govbr}{Link para o repositório do projeto}}. Para alcançar este propósito, 
...
\begin{itemize}
    ...
    \item Planejamento do Estudo de caso: descreve o protocolo utilizado para a condução do estudo de caso, detalhando seus objetivos, perguntas de pesquisa e atividades.
    \item Execução do Estudo de caso: apresenta a execução do planejamento, incluindo as etapas realizadas para a coleta e análise dos dados.
    \item Resultado do Estudo de Caso: explicita os resultados obtidos com a execução do estudo de caso, incluindo a análise dos dados coletados e a discussão dos achados.
...
\begin{itemize}
    ...
    \item Redação da Monografia: Escrita do texto da monografia, conforme a estrutura presente na Seção \ref{sec:estrutura-trabalho}.
...
Esta subseção, de forma semelhante à anterior, detalha as atividades previstas para a segunda etapa da monografia. A Figura \ref{fig:fluxograma-segunda-etapa} define o fluxo das atividades e a Figura \ref{fig:cronograma-tcc-2} o cronograma.

\begin{figure}[h]
    \centering
	\caption{Atividades da Segunda Etapa}
    \includegraphics[width=1.0\textwidth]{figuras/fluxograma-segunda-etapa.png}

    \label{fig:fluxograma-segunda-etapa}
	Fonte: Autor
\end{figure}
...
\begin{figure}[h]
    ...
    \includegraphics[width=1.0\textwidth]{figuras/cronograma-tcc-2.png}
...
\begin{itemize}
    \item Evolução do Trabalho: Revisão e aprimoramento do trabalho desenvolvido na primeira etapa, com base no opinião recebido durante a defesa. Além disso, foram adicionadas as atividades parcialmente concluídas na etapa anterior.
    \item Preparação e Coleta de Dados: Configuração do ambiente e das ferramentas necessárias para a coleta dos dados do estudo de caso.
    \item Tratamento dos Dados: Processamento e organização dos dados coletados, preparando-os para a análise.
    \item Documentação e Análise dos Resultados: Elaboração dos resultados, incluindo a análise dos dados e a discussão dos insights obtidos com a realização do estudo de caso.
    \item Redação da monografia: Continuação da escrita da monografia, de acordo com a estrutura da Seção \ref{sec:estrutura-trabalho}.
    \item Revisão: Correção final do trabalho.
    \item Apresentação: Defesa final do trabalho concluído.
\end{itemize}