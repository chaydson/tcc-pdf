...
Os artigos selecionados estão dispostos na Tabela \ref{tab:artigos-selecionados}. Para facilitar a compreensão do protocolo de seleção de artigos pode-se observar a Figura \ref{fig:fluxograma-artigos}, que ilustra todas as etapas explanadas anteriormente.


\setlist{nosep} % listas mais compactas dentro das células
...
\begin{longtable}{|p{5cm}|p{10cm}|}
    ...
    \multicolumn{2}{p{15cm}}{\centering \tablename\ \thetable{} -- Continuação} \\
    \hline
    \textbf{Item} & \textbf{Descrição} \\
    \hline
    \endhead
    ...
    \hline
    \multicolumn{2}{p{15cm}}{\centering Fonte: Autor} \\
    \endlastfoot

    % --- Linha: Questões Secundárias de Pesquisa ---
    ...
    % --- Linha: String de Busca ---
    String de Busca & Vide Tabela \ref{tab:picoadaptado} \\
    \hline

    % --- Linha: Critérios de Inclusão ---
...
\begin{longtable}{|p{1cm}|p{8cm}|p{3.5cm}|p{3cm}|}
    ...
    \hline
    \textbf{\textnumero{ N}} & \textbf{Título} & \textbf{Publicado em Revista} & \textbf{Validação Experimental} \\
    \hline
    \endfirsthead
    
    \multicolumn{4}{c}{\tablename\ \thetable{} -- Continuação} \\
    \hline
    \textbf{\textnumero} & \textbf{Título} & \textbf{Publicado em Revista} & \textbf{Validação Experimental} \\
    \hline
    \endhead
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Continuous Feedback: Obter opinião de maneira contínua e rápida é vital em ambientes de entrega contínua. Nessa abordagem, os problemas são rapidamente identificados, possibilitando que as informações cheguem depressa às equipes para que as medidas necessárias sejam tomadas. Os métodos tradicionais de coleta de dados e opinião são lentos demais para a agilidade requisitada em ambientes DevSecOps, e essa lentidão afeta diretamente a velocidade de localização e resolução dos problemas \cite{Rajapakse2022}.
    \end{itemize}
    ...
    \item Acompanhamento
    \begin{itemize}
        \item Monitoring e Logging: Muitas vezes, o registro e a documentação dos eventos relacionados à segurança são negligenciados pelas equipes, o que consiste em um grande erro. Registrar e monitorar fornece um opinião valioso que pode ser utilizado para tomar decisões estratégicas ou realizar auditorias \cite{Rajapakse2022}.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item Testes: SonarQube, FindBugs, Snyk, OWASP Dependency-Check, ZAP, Trivy, Detect Secrets, Asylo, StackHawk, JMeter, Selenium
\end{itemize}
