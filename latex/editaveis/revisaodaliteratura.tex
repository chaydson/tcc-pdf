...
Por essa razão, o protocolo PICO foi utilizado para guiar a elaboração de uma string de busca adequada às necessidades desta monografia. O protocolo, no entanto, precisou ser adaptado, pois sua origem é na medicina e nem todos os seus elementos se adequam área da Engenharia de Software \cite{pai_clinical_2004}.  Uma representação visual que facilita a compreensão do protocolo pode ser vista na Figura  \ref{fig:pico}.
 ...
 \todo[inline, color=pink]{Utilizar uma imagem com os termos em portugues. Você pode reutilizar e citar a figura 14 do tcc do Senna }
...
Assim, a definição de cada um dos elementos usados do modelo estão definidos na Tabela \ref{tab:pico}.
...
\todo[inline, color=pink]{Corrigir a string de busca com os termos que eu atualizei nos Resultados/Outcomes da tabela 2. Estavam incoerentes com aquele conjunto de termos que definimos em :  \href{https://docs.google.com/document/d/1eY4FqQtW9_JLSVFV5UFcHRBGDtQHDcpqyF6Ru-P7Kl4/edit?tab=t.0}{google docs Correções - TCC}}
...
foram lidos e os dados relevantes para responder as perguntas de pesquisa secundárias, foram extraídos em um formulário. As Tabelas \ref{tab:protocolo} e \ref{tab:protocolo-continuacao} contém o protocolo completo.
...
foi realizada uma busca manual com o objetivo de responder de maneira mais acertiva as perguntas de pesquisa, resultando em mais dois artigos \textcolor{blue}{selecionados. }\todo[color=yellow]{Inserir os .bibtex e corrigir a citação} \st{, Accelerate State of DevOps 2024 (DORA, 2024) e Quantitative DevSecOps Metrics for Cloud-Based Web Microservices (ZHANG; ZHANG, 2024)}, usados para compor o referencial teórico, totalizando 40 artigos.
Os artigos selecionados estão dispostos nas Tabelas 5, 6, 7 e 8. Para facilitar a compreensão do protocolo de seleção de artigos pode-se observar a Figura \ref{tab:artigos-selecionados-4}, que ilustra todas as etapas explanadas anteriormente.

\todo[inline, color=pink]{Corrigir a continuação de tabelas com o comando longtable. Veja um exemplo em: \url{https://www.overleaf.com/latex/examples/a-longtable-example/xxwzfxkxxjmc} }

\todo[inline, color=pink]{Traduzir todo protocolo para português. Já havíamos conversado sobre isso.}
...
\todo[inline, color=pink]{Uma observação geral sobre as imagens. Gere todas elas novamente com uma resolução de pelo menos 450-dpi . Por exemplo, se der um zoom nessa imagem 9, ela fica sem nitidz}